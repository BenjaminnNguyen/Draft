	...
	public void connect(String... database) {
		...
		if (mariaDb == null || mariaDb == "0"){
			if (database.length < 4) {
				...
				sid = System.getProperty("DBSid") != null ? System.getProperty("DBSid"):database[1];
				userName = System.getProperty("DBUser") != null ? System.getProperty("DBUser"):database[2];
				passWord = System.getProperty("DBPass") != null ? System.getProperty("DBPass"):database[3];
			}	
			try {
				info("Connect to DB " + db);
				Class.forName( "oracle.jdbc.driver.OracleDriver");
				String dbConnectString = "jdbc:oracle:thin:@//" + db + sid;
				if (database.length == 4){
					conn = DriverManager.getConnection(dbConnectString, userName,
							passWord);
				}else {
					if (database[4].equalsIgnoreCase("1")){
						conn = DriverManager.getConnection(dbConnectString, userName,
								encrypt(passWord));
					}
				}
			...
			connectMariaDB(database);
		}

	}
	public void connectSQL(String DBName , String... database) {
		if (database.length < 4) {
			db = DB;
			sid = SID;
			userName = USERNAME;
